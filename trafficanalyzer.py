...
class TrafficAnalyzer:
    ...
    def extract_features(self, packet, stats):
        ...
        return {
            ...
            'tcp_flags': int(packet[TCP].flags) if stats['type'] == 'TCP' else 0,
            'window_size': int(packet[TCP].window) if stats['type'] == 'TCP' else 0,
            'type': stats['type'],
            'source_ip': source_ip,
            'destination_ip': packet[IP].dst,
            ...
            'inter_arrival_times_mean': float(stats['inter_arrival_times_mean']),
            'inter_arrival_times_std': float(stats['inter_arrival_times_std']),
            'ratio_tcp_syn': float(stats['ratio_tcp_syn']),
            'ratio_tcp_rst': float(stats['ratio_tcp_rst']),
            'ratio_tcp_ack': float(stats['ratio_tcp_ack']),
            'is_tcp': int(stats['is_tcp']) if stats['is_tcp'] is not None else 0,
            'broadcast_or_multicast': int(broadcast_or_multicast),
            'is_private_to_private': int(is_private_to_private),
            'service_features': service_one_hot,
            'packet_size_zscore_rolling': float(stats['packet_size_zscore_rolling']),
            'packet_rate_zscore_rolling': float(stats['packet_rate_zscore_rolling'])
        }
